...
export default async function ReviewsPage() {
  ...
  const { data: rawSubmissions } = await supabase.from("submissions").select("id,status,written_response,project_link,video_link,learner_id,assessments(title),profiles(full_name,email)").in("status", ["submitted", "under_review", "revision_requested"]).order("submitted_at", { ascending: true });
