...
import { savePortfolioItem } from "@/app/portfolio/actions";
...
export default async function PortfolioPage() {
  ...
  const progress = calculatePortfolioProgress({
    ...
    hasPortfolioItem: (items ?? []).length > 0,
    hasVerifiedSkill: (verifiedSubmissions ?? []).length > 0,
  });
  return <main className="shell" style={{ paddingBlock: 30, display: "grid", gap: 20 }}>
    ...
    <section className="card" aria-label="Portfolio completion" style={{ padding: 22, display: "grid", gap: 12 }}>
      ...
      <ul style={{ margin: 0, paddingLeft: 20 }}><li>{profile.full_name && profile.biography ? "✓" : "○"} Complete your profile</li><li>{profile.selected_skills.length ? "✓" : "○"} Add at least one skill</li><li>{(items ?? []).length ? "✓" : "○"} Add a portfolio project</li><li>{(verifiedSubmissions ?? []).length ? "✓" : "○"} Get a skill verified by a mentor</li></ul>
    </section>
    <form action={savePortfolioItem} className="card" style={{ padding: 22, display: "grid", gap: 14 }}><div className="field"><label htmlFor="title">Project title</label><input id="title" name="title" required maxLength={160} /></div><div className="field"><label htmlFor="description">Description</label><textarea id="description" name="description" rows={4} maxLength={2000} /></div><div className="field"><label htmlFor="submission_id">Verified submission</label><select id="submission_id" name="submission_id"><option value="">Independent portfolio item</option>{(verifiedSubmissions ?? []).map((item: any) => <option key={item.id} value={item.id}>{item.assessments?.title ?? "Verified assessment"}</option>)}</select></div><label><input type="checkbox" name="is_public" /> Make public</label><button className="button" type="submit">Add portfolio item</button></form>
    <section style={{ display: "grid", gap: 14 }}>{(items ?? []).map((item: any) => <article className="card" style={{ padding: 20 }} key={item.id}><h2>{item.title}</h2><p>{item.description}</p><p>{item.submissions?.status === "verified" ? "✓ Verified evidence" : "Project evidence"} · {item.is_public ? "Public" : "Private"}</p></article>)}</section>
  </main>;
}
