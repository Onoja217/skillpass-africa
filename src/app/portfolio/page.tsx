...
export default async function PortfolioPage() {
  ...
  const verified = verifiedSubmissions ?? [];
  ...
  return <main className="shell" style={{ paddingBlock: 30, display: "grid", gap: 20 }}>
    ...
    <form action={savePortfolioItem} className="card" style={{ padding: 22, display: "grid", gap: 14 }}><h2>Add portfolio item</h2><div className="field"><label htmlFor="title">Project title</label><input id="title" name="title" required maxLength={160} /></div><div className="field"><label htmlFor="description">Description</label><textarea id="description" name="description" rows={4} maxLength={2000} /></div><div className="field"><label htmlFor="submission_id">Verified submission</label><select id="submission_id" name="submission_id"><option value="">Independent portfolio item</option>{verified.map((item: any) => <option key={item.id} value={item.id}>{item.assessments?.title ?? "Verified assessment"}</option>)}</select></div><label><input type="checkbox" name="is_public" /> Make public</label><button className="button" type="submit">Add portfolio item</button></form>
    <section style={{ display: "grid", gap: 14 }} aria-label="Your portfolio items">{portfolioItems.map((item: any) => <article className="card" style={{ padding: 20, display: "grid", gap: 14 }} key={item.id}><div><h2>{item.title}</h2><p>{item.description || "No description provided."}</p><p>{item.submissions?.status === "verified" ? "✓ Verified evidence" : "Project evidence"} · {item.is_public ? "Public" : "Private"}</p></div><details><summary>Edit portfolio item</summary><form action={updatePortfolioItem} style={{ display: "grid", gap: 12, marginTop: 14 }}><input type="hidden" name="id" value={item.id} /><div className="field"><label htmlFor={`title-${item.id}`}>Project title</label><input id={`title-${item.id}`} name="title" defaultValue={item.title} required maxLength={160} /></div><div className="field"><label htmlFor={`description-${item.id}`}>Description</label><textarea id={`description-${item.id}`} name="description" rows={4} defaultValue={item.description ?? ""} maxLength={2000} /></div><div className="field"><label htmlFor={`submission-${item.id}`}>Verified submission</label><select id={`submission-${item.id}`} name="submission_id" defaultValue={item.submission_id ?? ""}><option value="">Independent portfolio item</option>{verified.map((submission: any) => <option key={submission.id} value={submission.id}>{submission.assessments?.title ?? "Verified assessment"}</option>)}</select></div><label><input type="checkbox" name="is_public" defaultChecked={item.is_public} /> Make public</label><button className="button" type="submit">Save changes</button></form></details><form action={deletePortfolioItem}><input type="hidden" name="id" value={item.id} /><button type="submit">Delete portfolio item</button></form></article>)}</section>
  </main>;
}
