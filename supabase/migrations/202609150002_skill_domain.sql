...
as $$
  select exists (
    ...
    where id = auth.uid()
      and role in ('mentor', 'administrator')
  )
$$;
...
with check (public.is_mentor_or_administrator());
...
with check (
  public.is_mentor_or_administrator()
...
grant select on table public.categories, public.skills to anon, authenticated;
